...
| Window                                    | Average         |
| ----------------------------------------- | --------------- |
| Last 1 sprint (FE-S2)                     | 13 SP           |
| Last 2 sprints (FE-S1 + FE-S2)            | 13 SP/session   |
| Last 3 sprints (Sp12 + FE-S1 + FE-S2)     | 13 SP/session   |
| Last 5 sprints (Sp11–12 + FE-S1–S2)       | 15.2 SP/session |
| All-time backend (feature sprints Sp2–12) | 21.5 SP/session |
| Frontend only (FE-S1–S2)                  | 13 SP/session   |
...
- Frontend sprint velocity: **13 SP/sprint** so far (2 sprints, both auth/scaffold category — expected to rise in FE-S3+ as pages are content-heavy)
- Sprints 2–FE-S2 all single-session — `/brainstorm → /plan → /execute` workflow consistently effective
- Sprint 5: 96% — TM-3 deferred due to live PostgreSQL dependency
- Sprint 6: 95% — TM-3 deferred again (same reason)
- Sprints 7–FE-S2: 100% each
- **Frontend remaining:** 110 SP across 8 sprints (FE-S3 → FE-S10); estimated ~8 sessions at 13 SP/sprint
